...
$featured_tours_sql = $G['db']->q('SELECT t.id ,c.title,t.min_price,t.max_price,t.image ,t.start_country,t.rating,c.title ,c.desc ,c.url FROM ' . $GOGIES['dbprf'] . 'tours t LEFT JOIN ' . $GOGIES['dbprf'] . 'tours_contents c
ON t.id=c.tour_id WHERE  c.lang=\'' . $GOGIES['lang'] . '\' AND  t.status=\'1\' and
date(\'' . $tours_today . '\') between date(`f_start`) and date(f_finish)
   ORDER BY t.id DESC  LIMIT ' . $GOGIES['tours_settings']['latest_tours_number']);
// $tours_slides_limit=mysqli_num_rows($latest_tours_sql);
include $GOGIES['theme_path'] . '/modules/tours/featured_tours_block.php';
?>